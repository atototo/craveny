...
- [ ] Task 1: Git 레포지토리 초기화 (AC: 1)
  - [ ] `.gitignore` 파일 생성 (Python, Docker, IDE 패턴 포함)
  - [ ] 초기 커밋

- [ ] Task 2: 프로젝트 디렉토리 구조 생성 (AC: 6)
  - [ ] `backend/` 디렉토리 및 하위 폴더 생성 (crawlers/, llm/, telegram/, db/, scheduler/, api/, utils/)
  - [ ] `data/` 디렉토리 및 하위 폴더 생성 (news/, stock_prices/, logs/)
  - [ ] `scripts/`, `tests/`, `docs/` 디렉토리 생성
  - [ ] 각 Python 패키지에 `__init__.py` 추가

- [ ] Task 3: Docker Compose 설정 (AC: 2, 4)
  ...
  - [ ] `infrastructure/Dockerfile` 작성 (FastAPI 백엔드용)
  - [ ] 로컬 테스트: `docker-compose up` 실행 및 모든 서비스 정상 실행 확인

- [ ] Task 4: 환경 변수 설정 (AC: 3)
  - [ ] `.env.example` 파일 생성
...
- [ ] Task 5: Python 의존성 관리 (AC: 5)
  ...
  - [ ] `requirements-dev.txt` 작성 (pytest, black, flake8, mypy, isort)

- [ ] Task 6: README 문서 작성 (AC: 7)
  - [ ] 프로젝트 개요
  - [ ] 사전 요구사항 (Docker, Python 3.11+, API 키)
  - [ ] 로컬 환경 설정 단계
    - 레포지토리 클론
    - `.env` 파일 생성
...
- [ ] Task 7: 검증 및 테스트
  ...
  - [ ] Docker Compose 서비스들이 정상 실행되는지 확인
